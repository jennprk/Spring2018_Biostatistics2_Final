...
```{r}
plot(data$blood_pressure, exp(fit$residuals), ylab="Residuals")  ; abline(0, 0)

qqnorm(exp(fit$residuals));qqline(exp(fit$residuals),col="blue")
```

```{r}
se.lm <- summary(fit)$coef[,2]
se.lm

se.rb <- coeftest(fit, vcov = sandwich, df = 0)[,2]
se.rb

round(cbind(se.lm,se.rb,se.lm-se.rb),2)
```

```{r}
# check dfbetas
which(dfbetas(fit)[,2]< -2|dfbetas(fit)[,2] > 2)
which(dfbetas(fit)[,3]< -2|dfbetas(fit)[,3] > 2)
which(dfbetas(fit)[,4]< -2|dfbetas(fit)[,4] > 2)
which(dfbetas(fit)[,5]< -2|dfbetas(fit)[,5] > 2)
which(dfbetas(fit)[,6]< -2|dfbetas(fit)[,6] > 2)
which(dfbetas(fit)[,7]< -2|dfbetas(fit)[,7] > 2)
which(dfbetas(fit)[,8]< -2|dfbetas(fit)[,8] > 2)
which(dfbetas(fit)[,9]< -2|dfbetas(fit)[,9] > 2)
which(dfbetas(fit)[,10]< -2|dfbetas(fit)[,10] > 2)
```
